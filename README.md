...
```bash
# データ生成（output/ から viewer/public/data/ へ）
...
> **公開しないこと。** 警察地点の座標はTMT由来（譲渡禁止）のため、
> `viewer/public/data/` と `viewer/dist/` は `.gitignore` 対象。
> 公開するには事前にTMTへ「2次的著作物作成計画書」の提出・承認が必要（[DESIGN.md §3.5](DESIGN.md)）。

### 機能

| | 内容 |
|---|---|
| 時刻スライダー | 1時間モードは1ヶ月720ステップを通しでスクラブ、5分モードは日別288ステップ。`▶`で再生、`←→`キーで1ステップ送り、`Space`で再生/停止 |
| 機器フィルタ | 警察／国交省常設トラカン／国交省CCTVを個別に表示切替（**別系統の機器なので混ぜて見せない**） |
| 品質フィルタ | 常時ゼロ地点（89地点＝感知器故障の疑い）を隠せる |
| 表示 | 円の**塗り**が交通量、**輪郭色**が機器種別。欠測は灰色の小さな点 |
| ポップアップ | 地点名・その時刻の値・前後24ステップのスパークライン・故障疑いの警告 |
| 背景地図 | 地理院の最適化ベクトルタイル（淡色／標準／写真／白図）。ダークテーマは明度反転で生成 |
| 状態保存 | 位置・機器・モード・時刻をURL hashに保存（リロード・共有で復元） |

初期表示は札幌都心（`sapporo-micro-traffic-sim` のコードン区域）。「全体表示」で北海道全域。

### データ量

| ファイル | サイズ | 取得方法 |
|---|---|---|
| `stations.geojson` | 537 KB | 起動時 |
| `traffic_1h.json.gz` | 6.1 MB | 起動時に一括（720ステップ） |
| `traffic_5m/YYYYMMDD.json.gz` | 1.8 MB/日 | 日付切替時（計53MB・30日分） |

2,341地点を720ステップ動かしても滑らかに動くよう、ジオメトリは一度だけ投入し
`feature-state` の値だけを差分更新している。
