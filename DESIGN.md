...
1. **スコープ**: Phase 1 を札幌方面から始めるか、最初から全国か
2. **TMT詳細版の購入**: A（CSV座標のみ）か B（+Shapefile）か、1地域か全国か。
   購入データを使った成果物の共有範囲（社内/公開）の確認
3. **対象期間**: 何ヶ月分を遡って整備するか（警察typeBは公開中の月しか取れない
   可能性が高い → 早期にアーカイブ開始すべき）
4. **欠測の表現**: typeB で感知器停止時に「行が無い」のか「0が入る」のかの実データ確認
   （0と欠測の区別は品質上重要）
5. **リンク区分の意味**: typeB `リンク区分`（1/2…）のコード表の入手（DRM仕様）
6. **国交省1時間値と警察5分値集計の整合**: 端数・集計境界の扱い
